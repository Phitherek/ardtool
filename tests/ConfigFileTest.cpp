...
int main() {
    try {
        ...
        if(!p.valid()) {
            ...
            cout << "p.getName(): "  << p.getName() << endl << "p.getPath(): " << p.getPath()  << endl << "p.getEnv(): " << p.getEnv() << endl << "p.getRVM(): " << p.getRVM() << endl << "p.modulesEnd(): " << p.modulesEnd() << endl << "p.customEnd(): " << p.customEnd() << endl << "p.valid(): " << p.valid() << endl;
